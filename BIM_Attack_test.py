...
from art.attacks.evasion import ProjectedGradientDescent,FastGradientMethod,BasicIterativeMethod
...
save_dir = f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}"

print(Fore.YELLOW+Style.BRIGHT+f"當前時間: {current_time}")
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/", today)
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/", client_str)
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/", Choose_method)
getStartorEndtime("starttime",start_IDS,f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}")
...
def draw_confusion_matrix(y_true, y_pred, plot_confusion_matrix = False,epsilon = None,step_epsilon= None):
    ...
    if plot_confusion_matrix:
        ...
        # plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{client_str}_epochs_{num_epochs}_confusion_matrix.png")
        if epsilon == None and step_epsilon == None:
            plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{client_str}_epochs_{num_epochs}_epsilon_{epsilon}_confusion_matrix.png")
            plt.show()
        else:
            str_epsilon = f"epsilon_{epsilon}"
            str_step_epsilon = f"step_epsilon_{step_epsilon}"
            plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{str_epsilon}/{str_step_epsilon}/{client_str}_epochs_{num_epochs}_epsilon_{epsilon}_confusion_matrix.png")
            plt.close()
...
def test(net,testloader, start_time, client_str,plot_confusion_matrix):
    ...
    with torch.no_grad():
        for images, labels in testloader:
            ...
            with open(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/recall-baseline_{client_str}.csv", "a+") as file:
            ...
            with open(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/accuracy-baseline_{client_str}.csv", "a+") as file:
                if not header_written:
                    # file.write("標籤," + ",".join([str(i) for i in range(labelCount)]) + "\n")
                    header_written = True
                file.write(f"精確度,時間\n")
                ...
                report_df.to_csv(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/baseline_report_{client_str}.csv",header=True)
...
def BIM_Attack_evaluation(model, DEVICE, test_loader, classifier, attack, save_dir, epsilon, step_epsilon):
    ...
    # for batch_idx, (data, target) in tqdm.tqdm(enumerate(test_loader), desc=f'BIM - Batches:', total=len(test_loader)):
        data, target = data.to(DEVICE), target.to(DEVICE)
    ...
    print(Fore.RED + Style.BRIGHT+f'Average accuracy under BIM attack (ε={epsilon})(eps_step{step_epsilon}): {avg_accuracy:.4f}')
    ...
    str_epsilon = f"epsilon_{epsilon}"
    str_step_epsilon = f"step_epsilon_{step_epsilon}"
    generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/", str_epsilon)
    generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{str_epsilon}/", str_step_epsilon)
    # 標誌來跟踪是否已經添加了標題行
    header_written = False
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description='BIM Attack on CICIDS2017')
    ...
    # # 20241105 CIC-IDS2019 after do labelencode and minmax 75 25分 do BIM eps0.3
    # x_test = np.load(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/x_test_CICIDS2019_adversarial_samples_eps0.3.npy", allow_pickle=True)
    # y_test = np.load(f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/y_test_CICIDS2019_adversarial_labels_eps0.3.npy", allow_pickle=True)
    # 轉換為張量
    ...
    for epsilon in epsilons:
        for step_epsilons in step_epsilons_list:
            if step_epsilons <= epsilon:  # 確保步長不超過 epsilon
                attack = BasicIterativeMethod(
                    estimator=classifier,
                    eps=epsilon,
                    # eps_step=0.5,
                    eps_step=step_epsilons,# 每步的步長，可以根據需求調整
                    # max_iter=100,
                    max_iter=100,# BIM 的迭代次數
                    targeted=False
                )
            
                # 執行攻擊並評估
                acc, successful_attacks = BIM_Attack_evaluation(
        ...
        #紀錄結束時間
        end_IDS = time.time()
        getStartorEndtime("endtime",end_IDS,f"./Adversarial_Attack_Test/CICIDS2019/BIM_Attack/{today}/{current_time}/{client_str}/{Choose_method}")
