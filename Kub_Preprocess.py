...
generatefolder(filepath + "\\dataset_AfterProcessed\\", "Kub")
generatefolder(filepath + "\\dataset_AfterProcessed\\Kub\\", today)

ResotreTrainAndTestToCSVandReSplit("Kub",filepath)

def DoSpilthalfForiid(choose_merge_days):
    if choose_merge_days == "Kub":
        df_ALLtrain = pd.read_csv(filepath + "\\dataset_AfterProcessed\\Kub\\20240507\\Resplit_train_dataframes_20240507.csv")
