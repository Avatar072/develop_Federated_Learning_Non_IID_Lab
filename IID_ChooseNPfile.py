...
def CICIDS2018_IID_ChooseLoadNpArray(filepath,split_file, Choose_method):
    ...
    if split_file == 'total_train':
        ...
        if (Choose_method == 'normal'):
            ...
            # print(Fore.BLUE+Style.BRIGHT+"Loading CICIDS2018 a=0.1" +f"{split_file} with normal After Do labelencode and minmax 79 feature do feature mapping to 123 feature")
            ...
            x_train = np.load(filepath + "\\dataset_AfterProcessed\\CICIDS2018\\csv_data\\Dirichlet\\20250329\\alpha_0.5\\x_Dirichlet_client1_20250412.npy", allow_pickle=True)
            y_train = np.load(filepath + "\\dataset_AfterProcessed\\CICIDS2018\\csv_data\\Dirichlet\\20250329\\alpha_0.5\\y_Dirichlet_client1_20250412.npy", allow_pickle=True)
        ...
        if (Choose_method == 'normal'):
            ...
            # D:\develop_Federated_Learning_Non_IID_Lab\data\dataset_AfterProcessed\CICIDS2018\csv_data\Dirichlet\20250329
            print(Fore.BLUE+Style.BRIGHT+"Loading CICIDS2018 a=0.5" +f"{split_file} with normal After Do labelencode and minmax 79 feature do feature mapping to 123 feature")
            x_train = np.load(filepath + "\\dataset_AfterProcessed\\CICIDS2018\\csv_data\\Dirichlet\\20250329\\alpha_0.5\\x_Dirichlet_client2_20250412.npy", allow_pickle=True)
            y_train = np.load(filepath + "\\dataset_AfterProcessed\\CICIDS2018\\csv_data\\Dirichlet\\20250329\\alpha_0.5\\y_Dirichlet_client2_20250412.npy", allow_pickle=True)
...
def TONIOT_IID_ChooseLoadNpArray(filepath,split_file, Choose_method):
    if split_file == 'total_train':
        print("Training with total_train")
        if (Choose_method == 'normal'):
            # 20240520 EdgeIIoT after do labelencode and minmax chi_square45 75 25分
        ...
        if (Choose_method == 'normal'):
            ...
            # x_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.1\\x_Dirichlet_client1_20250414.npy", allow_pickle=True)
            # y_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.1\\y_Dirichlet_client1_20250414.npy", allow_pickle=True)

            # 20250414 TONIOT 使用a=0.5 after do labelencode 44 feature and all featrue minmax 75 25分
            # 44 feature use Label meraged BaseLine data do feature mapping to 123 feature
            # D:\develop_Federated_Learning_Non_IID_Lab\data\dataset_AfterProcessed\TONIOT\Dirichlet\20250414
            print(Fore.BLUE+Style.BRIGHT+"Loading TONIOT a=0.5" +f"{split_file} with normal After Do labelencode and minmax 44 feature do feature mapping to 123 feature")
            x_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.5\\x_Dirichlet_client1_20250414.npy", allow_pickle=True)
            y_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.5\\y_Dirichlet_client1_20250414.npy", allow_pickle=True)
        ...
        if (Choose_method == 'normal'):
            ...
            # D:\develop_Federated_Learning_Non_IID_Lab\data\dataset_AfterProcessed\TONIOT\Dirichlet\20250414
            # print(Fore.BLUE+Style.BRIGHT+"Loading TONIOT a=0.1" +f"{split_file} with normal After Do labelencode and minmax 44 feature do feature mapping to 123 feature")
            # x_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.1\\x_Dirichlet_client2_20250414.npy", allow_pickle=True)
            # y_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.1\\y_Dirichlet_client2_20250414.npy", allow_pickle=True)

            # 20250414 TONIOT 使用a=0.5 after do labelencode 44 feature and all featrue minmax 75 25分
            # 44 feature use Label meraged BaseLine data do feature mapping to 123 feature
            # D:\develop_Federated_Learning_Non_IID_Lab\data\dataset_AfterProcessed\TONIOT\Dirichlet\20250414
            print(Fore.BLUE+Style.BRIGHT+"Loading TONIOT a=0.5" +f"{split_file} with normal After Do labelencode and minmax 44 feature do feature mapping to 123 feature")
            x_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.5\\x_Dirichlet_client2_20250414.npy", allow_pickle=True)
            y_train = np.load(filepath + "\\dataset_AfterProcessed\\TONIOT\\Dirichlet\\20250414\\alpha_0.5\\y_Dirichlet_client2_20250414.npy", allow_pickle=True)
...
def ChooseLoad_class_names(str_choose_dataset):
    ...
    if str_choose_dataset == 'CICIDS2017':
        ...
        class_names_local = {0: 'Benign', 1: 'Backdoor', 2: 'DDoS', 3: 'DoS', 
