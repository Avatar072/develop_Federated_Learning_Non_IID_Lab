...
save_dir = f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}"

print(Fore.YELLOW+Style.BRIGHT+f"當前時間: {current_time}")
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/", today)
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/", client_str)
generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/", Choose_method)
getStartorEndtime("starttime",start_IDS,f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}")
...
def draw_confusion_matrix(y_true, y_pred, plot_confusion_matrix = False,epsilon = None):
    ...
    if plot_confusion_matrix:
        ...
        # plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{client_str}_epochs_{num_epochs}_confusion_matrix.png")
        if epsilon == None:
            plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{client_str}_epochs_{num_epochs}_epsilon_{epsilon}_confusion_matrix.png")
        else:
            str_epsilon = f"epsilon_{epsilon}"
            plt.savefig(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/{str_epsilon}/{client_str}_epochs_{num_epochs}_epsilon_{epsilon}_confusion_matrix.png")
...
def test(net,testloader, start_time, client_str,plot_confusion_matrix):
    ...
    with torch.no_grad():
        for images, labels in testloader:
            ...
            with open(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/recall-baseline_{client_str}.csv", "a+") as file:
            ...
            with open(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/accuracy-baseline_{client_str}.csv", "a+") as file:
                if not header_written:
                    # file.write("標籤," + ",".join([str(i) for i in range(labelCount)]) + "\n")
                    header_written = True
                file.write(f"精確度,時間\n")
                ...
                report_df.to_csv(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/baseline_report_{client_str}.csv",header=True)
...
def JSMA_attack_evaluation(model, DEVICE, test_loader, classifier, attack, save_dir, epsilon):
    ...
    print(Fore.RED + Style.BRIGHT+f'Average accuracy under JSMA attack (ε={epsilon}): {avg_accuracy:.4f}')
    ...
    str_epsilon = f"epsilon_{epsilon}"
    generatefolder(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}/", str_epsilon)
    # 標誌來跟踪是否已經添加了標題行
    header_written = False
...
def main():
    ...
    # # 20241105 CIC-IDS2019 after do labelencode and minmax 75 25分 do JSMA eps0.3
    # x_test = np.load(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/x_test_CICIDS2019_adversarial_samples_eps0.3.npy", allow_pickle=True)
    # y_test = np.load(f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/y_test_CICIDS2019_adversarial_labels_eps0.3.npy", allow_pickle=True)
    # 轉換為張量
    ...
    #     )
        ...
        #紀錄結束時間
        end_IDS = time.time()
        getStartorEndtime("endtime",end_IDS,f"./Adversarial_Attack_Test/CICIDS2019/JSMA_Attack/{today}/{current_time}/{client_str}/{Choose_method}")
